...
class PlayScene extends Phaser.Scene {
  ...
  update(time, delta) {
    ...
    if (Input.left) {
      ...
      p.body.velocity.x *= (p.body.touching.down ? PHYS.FRICTION_GROUND : PHYS.FRICTION_AIR);
    }
    if (p.body.velocity.x > PHYS.MOVE_MAX) p.body.velocity.x = PHYS.MOVE_MAX;
    if (p.body.velocity.x < -PHYS.MOVE_MAX) p.body.velocity.x = -PHYS.MOVE_MAX;

    // jump (jumpPressed is true exactly once per press, set by Input.tick())
    ...
    // variable-height jump: cut velocity when player releases jump
