...
export const CFG = {
  ...
  bosses: {
    ...
    // Boss 5: "mmb" (Mecha MB) — Data11 row 6 / Levels F6 (Level 5).
    // "an exact copy of mb but made of metal the same size same shape ...
    //  3000hp 3 phases".
    //   p1: laser arena fight  (3000 → 2000)
    //   p2: race / chase section (Apple King's laser wall; MMB isn't a
    //       tickable boss here, the player races to the finish)
    //   p3: giant form with a weak-circle on his chest (2000 → 0)
    mmb: {
      hp: 3000, phase2At: 2000 / 3000, phase3At: 0.0,   // p2 transitions on hp; p3 on race-clear
      radius: 0.7,                                       // matches MB's own radius
      contactDmg: 12,
      laserDmg: 14,
      lightningDmg: 18,
      crushDmg: 22,
...
// ---------------- Level 5 — MMB + Apple King race, save Naomi ----------------
// Data11 Levels F6 (NEW). Three-phase narrative level:
//   intro    — cutscene: MB blows up spaceship, finds MMB
//   p1       — MMB shoots colored lasers (red/blue/silver/gold). MB attacks.
//              Until MMB takes 1000 dmg (3000 → 2000).
//   p2 race  — MMB runs to creator Apple King. Apple King fires an instakill
//              laser wall from behind. MB races forward, dodging spikes; the
//              finish line closes on MMB. No MB attacks during the race.
//   p3       — MMB returns giant-sized with a weak-circle on his chest.
//              Shoots lightning, tries to crush MB. Beat him (2000 → 0).
//   rescue   — Naomi card via the blue portal. No inspirations in this level.
export const L5 = {
  kind: 'mmb',
  dog: 'naomi',
  boss: 'mmb',

  // Phase 1 arena (a metallic spaceship-interior).
  arenaRadius: 28,
  playerSpawn: [0, 0, 12],
  bossSpawn:   [0, 0, -8],

  // Phase 2 race section — a long corridor MB runs forward through.
  // Data12 F6: "mmb is in the race its 1000m long. 10% of the time mmb will
  // turn on a energy sheild (lasts 10s) so you cant touch mmb without takng
  // damage." MMB is now a live participant in the race (not just cosmetic):
  // he runs alongside MB and periodically raises a shield that hurts on touch.
  raceLength: 80,             // total z-distance (world units) start → finish
  raceDistanceMeters: 1000,   // Data12: the race is "1000m long" (HUD readout)
  raceWidth:  10,             // half-width on x for strafing
  raceForwardSpeed: 6.0,      // MB's auto-forward speed during the race
  laserWallSpeedStart: 2.0,   // initial advance rate of the death wall
  laserWallSpeedRamp:  0.04,  // accelerates over time so it catches you if you stall
  raceSpikes: [               // [x, zOffsetFromStart]
    [ 2,  10], [-2,  18], [ 3,  24], [-4,  32],
    [ 1,  40], [-3,  48], [ 4,  56], [ 0,  64],
    [-2,  72],
  ],
  // MMB's racing energy shield (Data12 F6).
  raceShieldChance:   0.10,   // 10% chance per check window to raise the shield
  raceShieldCheck:    1.0,    // seconds between shield-activation rolls
  raceShieldDuration: 10.0,   // "lasts 10s"
  raceMmbContactDmg:  16,     // damage when touching MMB while his shield is up
  raceMmbForwardSpeed: 6.4,   // MMB runs a touch faster than MB so he leads

  // Phase 3 giant arena (reuse phase 1 space, larger boss).
  portalSpawn: [0, 2.6, -22],
  // Data12 F6: "thers stairs that lead up to mmb you climb th stairs and stomp
  // on mb." A staircase in the giant arena MB climbs to reach stomp height.
  // Reuses the proven ladder climb-zone (player.js) for the actual ascent.
  stairs: {
    pos:    [0, 0, 0],   // arena floor position of the staircase base
    height: 5.0,         // climb to ~giant-MMB head height so a stomp connects
    steps:  9,
    width:  3.2,
  },
};
