...
import { CFG, L1, L2, L3, L5 } from './config.js';
...
import { startMmbLevel } from './mmbLevel.js';
...
//  bar resets". Data14 tightens this to 2 tries. We track the carry across
//  rebuildScene() calls in two module-level slots: G.l4_triesUsed and
//  G.l4_bossHpCarry. On death the level passes its current boss HP up via
//  opts.onDeath(bossHp). If we haven't burned through all the tries, the next
//  start re-spawns the boss at the carried HP; on the final failure we reset
//  both slots and the boss reappears at full HP.
// ============================================================
const L4_MAX_TRIES = 2;
  ...
  currentLevel = startSpaceLevel(scene, engine, player, {
    ...
    onDeath: (carriedBossHp) => {
      ...
      if (G.l4_triesUsed >= L4_MAX_TRIES) {
        // Burned through all tries — boss HP resets, counter resets.
        G.l4_triesUsed = 0;
        G.l4_bossHpCarry = null;
      } else {
...
// ============================================================
//  LEVEL 5 — Mecha-MB + Apple King race, save Naomi
// ============================================================
function startMmb() {
  rebuildScene();
  hud.show();
  G.phase = 'mmb'; G.score = 0; G.levelKey = 'L5';
  const data = GAME_DATA.levels[4];
  const augmented = { ...L5, levelNumber: data.number, levelName: data.name };
  currentLevel = startMmbLevel(scene, engine, player, augmented, {
    initialScore: 0,
    onComplete: (finalScore) => {
      G.score = finalScore;
      showComplete('LEVEL 5 COMPLETE');
    },
    onDeath: () => startMmb(),
  });
  engine.runRenderLoop(renderTick);
...
click('btn-l5', () => { hideLevelSelect(); startMmb(); });
click('replayBtn', () => {
