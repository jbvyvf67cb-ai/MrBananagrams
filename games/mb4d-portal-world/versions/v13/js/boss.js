...
function buildMmb(scene, root, def, R) {
  ...
  // Race ENERGY SHIELD (Data12 F6) — a translucent glowing bubble around MMB.
  // Hidden by default; mmbLevel.js toggles it during the phase-2 race. While
  // it's up, touching MMB damages MB ("you cant touch mmb without takng damage").
  const shield = facetedSphere(scene, 'mmbShield', R + 0.9, 2);
  shield.parent = root;
  shield.position.y = 1.0;
  const shieldMat = mat(scene, 'mmbShieldMat', C(0.30, 0.70, 1.0), { emissive: C(0.25, 0.65, 1.0) });
  shieldMat.alpha = 0.32;
  shield.material = shieldMat;
  shield.isPickable = false;
  shield.setEnabled(false);

  return {
    kind: 'mmb', root, body, skin: chromeBody, visor, weakCircle, shield,
    radius: R, headY: 1.9,
    hp: def.hp, maxHp: def.hp, alive: true,
    ...
    crushTimer: 0,
    lightningTimer: 0,
    // v13 (Data13 F6): "once your half way to the staairs mb stops shotin
    // lightning". mmbLevel.js flips this true when MB reaches the stair
    // midpoint in the giant phase; tickMmb then halts the lightning attack.
    lightningStopped: false,
    raceFinished: false,
    giantMode: false,
    // Race-shield state (Data12). Driven by mmbLevel.js during p2.
    shieldUp: false,
    shieldTimer: 0,      // remaining seconds the shield stays up
    shieldCheck: 0,      // countdown to the next activation roll
...
function tickMmb(b, dt, player, cb) {
  ...
  if (b.phase === 3) {
    ...
    // Lightning bolts strike near MB — but only until MB is halfway to the
    // stairs. v13 (Data13 F6): "once your half way to the staairs mb stops
    // shotin lightning". mmbLevel.js sets b.lightningStopped at that point.
    b.lightningTimer = (b.lightningTimer || 0) - dt;
    if (b.lightningTimer <= 0 && !b.lightningStopped) {
