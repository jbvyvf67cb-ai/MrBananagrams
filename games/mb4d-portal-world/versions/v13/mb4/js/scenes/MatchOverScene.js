...
class MatchOverScene extends Phaser.Scene {
  ...
  create() {
    // v6: MatchOver now only appears on a LOSS (a Hipball win routes straight
    // into BattleScene). No parent bridge here — the 3D handoff happens from
    // BattleScene via the 'mb4-portal' message after the boss is beaten.
    ...
    // Announcer call — winner perspective
    Audio4.speak('you_win', 1.4);

    // Stop match music; play win/lose sting if files exist.
    if (window.Music4) {
      Music4.stop();
      // P1 won → 'win' for P1's perspective. We always treat as win — the
      // file is just an end-of-match flourish, not perspective-specific.
      Music4.play('win');
    }
  }
