...
(function () {
  ...
  function tick() {
    ...
    // R = "shot peel" — added in v8 per the Data7 Levels F2 rewrite.
    updatePlayer(p1, {
  ...
  function updatePlayer(p, raw) {
    ...
    p.peelPressed  = raw.peel  && !p._prev.peel;
    p.left = raw.left; p.right = raw.right;
    p.fwd  = raw.fwd;  p.back  = raw.back;
    p.jump = raw.jump;
    p.hip = raw.hip; p.knee = raw.knee; p.elbow = raw.elbow; p.peel = raw.peel;
    p._prev = { jump: raw.jump, hip: raw.hip, knee: raw.knee, elbow: raw.elbow, peel: raw.peel };
  }
