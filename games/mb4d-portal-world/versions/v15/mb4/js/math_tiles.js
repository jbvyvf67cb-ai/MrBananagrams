...
class MathTileManager {
  ...
  _checkPlayerOverlap(player, now) {
    ...
    if (player.slipUntil && now < player.slipUntil) return;   // slipping = immune
    for (const tile of this.tiles) {
      if (tile.resolved) continue;
  ...
  _resolveTile(tile, player, now) {
    ...
    if (correct) {
      // INSTANT POWERUP — pick one at random from the pool
      Audio4.mathRight();
      this._flashTile(tile, 0x4ea886);
      this.scene._mathFloatText(tile, '\u2713', '#4ea886');
      const pool = MB4.POWERUP_POOL;
      const kind = pool[Math.floor(Math.random() * pool.length)];
      this.scene._applyPowerUp(player, kind);
    } else {
      // SLIP AND FALL — player can't move/strike for SLIP_MS
      Audio4.mathWrong();
      this._flashTile(tile, 0xc0392b);
      this.scene._mathFloatText(tile, '\u2717', '#c0392b');
      this.scene._slipPlayer(player);
    }
