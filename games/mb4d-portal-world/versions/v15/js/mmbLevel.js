...
export function startMmbLevel(scene, engine, player, spec, opts) {
  ...
  function enterGiantBoss() {
    ...
    boss.maxHp = 3000;
    boss.alive = true;
    // Data13 F6: lightning fires until MB is "half way to the staairs".
    // Record MB's starting distance to the stair base so the tick can detect
    // the halfway point; lightning is on (boss.lightningStopped = false) until then.
    boss.lightningStopped = false;
    state.stairStartDist = Math.hypot(
      spec.playerSpawn[0] - spec.stairs.pos[0],
      spec.playerSpawn[2] - spec.stairs.pos[2]);
    state.lightningHalted = false;
  ...
  function tick() {
    ...
    // Data13 F6: in the giant phase, once MB is "half way to the staairs" MMB
    // stops shooting lightning (crush slams + contact still apply).
    if (state.phase === 'p3' && boss && boss.alive && !state.lightningHalted) {
      const pp = player.pos();
      const distToStairs = Math.hypot(pp.x - spec.stairs.pos[0], pp.z - spec.stairs.pos[2]);
      if (distToStairs <= state.stairStartDist * 0.5) {
        state.lightningHalted = true;
        boss.lightningStopped = true;
        hud.banner('Lightning stops — climb the stairs!', 1800);
      }
    }
