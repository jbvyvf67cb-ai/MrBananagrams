...
import { createHipballScene, updateHipball, setEnabled as hbSetEnabled } from './hipball.js';
...
function startHipball() {
  ...
  hud.showRayas();
  hud.setRayas(0, 0, CFG.hipball.rayasToWin);
  hud.setTouchPhase('hipball');
  G.score = 0;
  hud.setScore(0);
  hud.setHp(player.hp, player.maxHp);
  hud.banner('LEVEL 1 — Hipball! Strike with Z (crunch) · X (slam) · C (hip)', 3200);
}
...
scene.onBeforeRenderObservable.add(() => {
  ...
  if (G.phase === 'title' || G.phase === 'hb-transition') { input.endFrame(); return; }
