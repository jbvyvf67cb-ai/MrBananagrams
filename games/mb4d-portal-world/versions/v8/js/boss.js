...
//   5: drops BOMBS from above every CFG.bossInterval; also fires occasionally
function tickParticleColider(b, dt, player, cb) {
  ...
  // Fireball cadence per phase.
  const cadence = b.phase === 2 ? 2.5 : b.phase === 3 ? 2.0 : 1.6;
  if (b.timer <= 0) {
    const shots = b.phase === 4 ? 3 : (b.phase >= 3 ? 2 : 1);
