...
import { buildForest, buildLadder } from './world-forest.js';
...
export function startForestLevel(scene, engine, player, spec, opts) {
  ...
  // v8 (Data7): the acorn tree now has a ladder. Place it on the side of
  // the tallest tree facing the player spawn so MB can climb to the acorn.
  const ladders = [];
  if (tallest) {
    const sx = Math.sign(spec.playerSpawn[0] - tallest.x) || 1;
    const sz = Math.sign(spec.playerSpawn[2] - tallest.z) || 1;
    const ladderH = tallest.h + 0.4;
    ladders.push(buildLadder(scene, {
      x: tallest.x, z: tallest.z, height: ladderH,
      offX: sx * 0.9, offZ: sz * 0.9,
    }));
  }
  ...
  return {
    dispose() {
      ...
      for (const l of ladders) l.root.dispose();
