...
export function startSpaceLevel(scene, engine, player, opts) {
  ...
  function spawnBoss() {
    try {
      ...
      const triesLeft = (opts?.triesMax ?? 4) - (opts?.triesUsed ?? 0);
      const tryNote = triesLeft < (opts?.triesMax ?? 4)
        ? ' · try ' + ((opts?.triesUsed ?? 0) + 1) + ' / ' + (opts?.triesMax ?? 4)
        : '';
  ...
  function onPlayerHit(dmg) {
    if (damagePlayer(player, dmg)) {
      ...
      if (!player.alive && state.phase !== 'dead') {
        ...
        // (up to 4 tries). Deaths before the boss spawn pass null and
