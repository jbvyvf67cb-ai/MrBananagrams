...
export function updatePlayer(p, dt, world, input, cb, nowSec) {
  ...
  // 3a) Detect ladder zone first — needed so Space climbs (Data10 F3)
  // instead of jumping when MB is on a rung.
  p.onLadder = false;
  if (world.ladders) {
  ...
  // 3b) Jump + double jump (Space, then Space again mid-air).
  // Suppressed while on a ladder so Space climbs instead.
  if (input.pressed('jump') && !p.crouching && !p.onLadder) {
    if (p.grounded) {
  ...
  // 3c) Ladder climb. Data10 F3 added "space to climb ladder", so Space is
  // now the primary climb key alongside ↑. ↓ still climbs down. Gravity is
  // suppressed while on the ladder; stepping sideways off drops MB back
  // into normal physics.
  if (p.onLadder) {
    const climbUp   = input.down('fwd') || input.down('jump');
    const climbDown = input.down('back');
    if (climbUp)        p.vel.y =  4.5;
    else if (climbDown) p.vel.y = -3.5;
    else                p.vel.y =  0;
  }
