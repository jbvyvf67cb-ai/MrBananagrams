...
class BootScene extends Phaser.Scene {
  ...
  create() {
    ...
    this._makeBananagram('p1_fallen', C.p1, C.p1Skin, 'fallen');
    ...
    this._makeBananagram('p2_fallen', C.p2, C.p2Skin, 'fallen');
    ...
    // ---- Stone hoop (mounted vertically on back wall, viewed from the SIDE) ----
    // The ring is in a vertical plane facing us — narrow horizontally, tall vertically.
    // This matches how the Mesoamerican stone hoops were actually mounted.
    {
      const g = this.make.graphics({ x: 0, y: 0, add: false });
      const cx = 32, cy = 60;
      // Outer stone ring (a fat ellipse)
      g.fillStyle(C.stoneLt, 1); g.fillEllipse(cx, cy, 36, 60);
      g.fillStyle(C.stoneMid, 1); g.fillEllipse(cx, cy, 28, 50);
      // Inner hole (the ring opening). Fill solid here — the scene draws a
      // backdrop-colored disc behind so it reads as a real hole.
      g.fillStyle(0x100610, 1); g.fillEllipse(cx, cy, 18, 38);
      // Glyph notches around the ring
      g.fillStyle(C.glyphAccent, 1);
      g.fillRect(cx - 2, 6, 4, 4);          // top
      g.fillRect(cx - 2, 110, 4, 4);        // bottom
      g.fillRect(cx - 18, cy - 2, 4, 4);    // left
      g.fillRect(cx + 14, cy - 2, 4, 4);    // right
      // Mounting peg into the wall (the carved tongue holding the ring)
      g.fillStyle(C.stoneDk, 1);
      g.fillRect(cx - 4, cy - 4, 8, 8);
      g.generateTexture('hoop', 64, 120);
      g.destroy();
    }
  ...
  _makeBananagram(key, bodyColor, skinColor, pose) {
    ...
    if (pose === 'idle') {
      ...
      // Flat on back: arms splayed wide, legs out, X eyes, dazed mouth.
      g.fillRect(-2, 30, 10, 6);      // left arm flung out
      g.fillRect(28, 30, 10, 6);      // right arm flung out
      g.fillRect(6, 40, 6, 8);        // left leg out
      g.fillRect(24, 40, 6, 8);       // right leg out
    }

    // Fallen pose: paint X eyes over the regular eyes + tongue out.
    if (pose === 'fallen') {
      // Block out the regular eyes with skin color
      g.fillStyle(bodyColor, 1);
      g.fillRect(11, 18, 4, 5); g.fillRect(21, 18, 4, 5);
      // X eyes — two crossed lines each side
      g.fillStyle(0x000000, 1);
      // left X (centered around 13, 20)
      g.fillRect(11, 18, 5, 1); g.fillRect(11, 22, 5, 1);
      g.fillRect(11, 19, 1, 1); g.fillRect(15, 19, 1, 1);
      g.fillRect(11, 21, 1, 1); g.fillRect(15, 21, 1, 1);
      g.fillRect(13, 20, 1, 1);
      // right X (centered around 23, 20)
      g.fillRect(21, 18, 5, 1); g.fillRect(21, 22, 5, 1);
      g.fillRect(21, 19, 1, 1); g.fillRect(25, 19, 1, 1);
      g.fillRect(21, 21, 1, 1); g.fillRect(25, 21, 1, 1);
      g.fillRect(23, 20, 1, 1);
      // tongue sticking out
      g.fillStyle(0xc0392b, 1);
      g.fillRect(16, 30, 4, 5);
    }
