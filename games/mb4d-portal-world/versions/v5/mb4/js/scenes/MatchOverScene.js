...
class MatchOverScene extends Phaser.Scene {
  ...
  create() {
    // v5 BRIDGE: when MB4 is embedded inside MB4D Level 1, tell the parent
    // page the match ended so it can transition to the wave phase.
    // winner === 0 → MB; winner === 1 → CPU.
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({
        type: 'mb4-result',
        winner: this.winner === 0 ? 'mb' : 'opponent',
        score: this.score,
        how: this.how,
      }, '*');
    }
