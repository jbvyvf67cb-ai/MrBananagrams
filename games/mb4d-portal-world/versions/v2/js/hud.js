...
function buildTouch() {
  ...
  // Left side: turn ↺ ↻, plus arrow movement.
  ...
  pad.append(mk('▲', 'jump', 'tap'), mk('◀', 'strafeL', 'hold'), mk('▶', 'strafeR', 'hold'), mk('▼', 'back', 'hold'));
  // Right side: forward + attacks.
  const acts = document.createElement('div'); acts.className = 'tacts';
  acts.append(mk('GO', 'fwd', 'hold'), mk('PEEL', 'shoot', 'tap'), mk('5×', 'special', 'tap'));
  wrap.append(turns, pad, acts);
}
