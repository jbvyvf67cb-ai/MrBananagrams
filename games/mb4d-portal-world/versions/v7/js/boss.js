...
export function createBoss(scene, kind, pos) {
  ...
  return buildPortalprotector(scene, root, def, R);   // fallback, Level 1 style
}
...
export function damageBoss(b, amt) {
  ...
  if (main && main.material) {
    ...
    setTimeout(() => {
      if (b.alive && main.material) {
        ...
        else if (b.kind === 'babyblue')  main.material.emissiveColor = C(0, 0, 0);
  ...
  let enteredPhase = 0;
  if (def.phase3At && oldPhase < 3 && b.hp <= def.hp * def.phase3At) { b.phase = 3; enteredPhase = 3; }
