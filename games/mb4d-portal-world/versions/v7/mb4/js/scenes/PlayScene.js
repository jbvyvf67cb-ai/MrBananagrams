...
class PlayScene extends Phaser.Scene {
  ...
  _endMatch(winnerIdx, how) {
    ...
    // MB4D v6 (Level 1): winning the Hipball match doesn't end the level — it
    // leads straight into the 2D battle (2 enemy waves + portalprotector) on
    // this same court, per the Data6 spec. A LOSS still goes to MatchOver so
    // the player can retry. (In local/online modes we keep the classic flow.)
    if (winnerIdx === 0 && this.mode === 'sp') {
      this.time.delayedCall(900, () => {
        this.scene.start('Battle', { difficulty: this.difficulty, score: 0 });
      });
      return;
    }
