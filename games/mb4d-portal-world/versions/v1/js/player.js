...
// Mr Bananagrams — faceted, banana-yellow. Kinematic movement (manual gravity)
// for tight platformer feel; multi-tap jump (single/double/triple) per spec.
export function createPlayer(scene) {
  ...
  return {
    ...
    invuln: 0, peelCd: 0, specialCd: 0, alive: true,
    pos: () => root.position,
  };
}
...
export function updatePlayer(p, dt, cam, world, input, cb) {
  ...
  const fwd = cam.getForwardRay().direction.clone(); fwd.y = 0;
  if (fwd.lengthSquared() < 1e-5) fwd.set(0, 0, 1);
  fwd.normalize();
  const right = BABYLON.Vector3.Cross(fwd, BABYLON.Axis.Y).normalize();

  const wish = new BABYLON.Vector3(0, 0, 0);
  if (input.down('up')) wish.addInPlace(fwd);
  if (input.down('down')) wish.subtractInPlace(fwd);
  if (input.down('right')) wish.addInPlace(right);
  if (input.down('left')) wish.subtractInPlace(right);

  p.crouching = input.down('crouch') && p.grounded;
  const speed = P.moveSpeed * (p.crouching ? P.crouchSpeedMul : 1);

  let target = new BABYLON.Vector3(0, 0, 0);
  if (wish.lengthSquared() > 0) {
    wish.normalize();
    target = wish.scale(speed);
    p.yaw = Math.atan2(wish.x, wish.z);   // face travel direction
  }
  ...
  // Visuals: yaw + crouch squash.
  p.root.rotation.y = p.yaw;
  const sq = p.crouching ? 0.6 : 1;
  p.body.scaling.y = BABYLON.Scalar.Lerp(p.body.scaling.y, 1.25 * sq, 0.3);

  // Shooting.
  p.peelCd = Math.max(0, p.peelCd - dt);
  p.specialCd = Math.max(0, p.specialCd - dt);
  const muzzle = pos.add(facingVec(p.yaw).scale(0.7)); muzzle.y += 1.1;
  if (input.pressed('special') && p.specialCd === 0) {
  ...
  } else if (input.pressed('shoot') && p.peelCd === 0) {
    cb.shoot(muzzle, facingVec(p.yaw));
