...
class BattleScene extends Phaser.Scene {
  ...
  _damageTarget(t, dmg, knockDir) {
    ...
    if (t === this.boss) {
      if (!t.phase2 && t.hp <= t.maxHp * BATTLE.BOSS.phase2At) {
        t.phase2 = true; this._banner('PHASE 2!', 1200);
      }
  ...
  _updateBoss(dt, time) {
    ...
    // v9 (Data8 Levels F2): "the boss cant shot anything". Portalprotector
    // is melee-only now — no fireballs in any phase, just charge / contact.
    // Phase 2 still triggers (faster movement, stronger contact), it just
    // doesn't throw projectiles.
  }
  ...
  // ---------------- fireballs ----------------
  _spawnFireball(x, depth, tx, tdepth, speed, dmg) {
    const ang = Math.atan2((tdepth - depth) * 300, tx - x);
    const sprite = this.add.image(this._perspectiveX(x, depth), this._floorY(depth) - 60, 'fireball').setDepth(210);
    this.fireballs.push({
      x, depth, height: 50,
