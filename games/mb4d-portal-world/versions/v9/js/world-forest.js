...
function makeTree(scene, x, z, h, trunkMat, foliageMat, variant, isTallest) {
  ...
  return { root, x, z, h, isTallest, trunkH };
}

// Build a wooden ladder on the side of a tree. Two vertical rails plus
// `rungCount` horizontal rungs. The ladder occupies a small column at
// (x+offX, z+offZ); player.js checks ladders for climb input.
export function buildLadder(scene, opts) {
  const { x, z, height, offX = 0, offZ = 0 } = opts;
  const root = new BABYLON.TransformNode('ladder', scene);
  root.position.set(x + offX, 0, z + offZ);

  const woodMat = mat(scene, 'ladderWood', C(0.55, 0.34, 0.18));
  // Two vertical rails (40-sided thin cylinders).
  for (const sx of [-0.35, 0.35]) {
    const rail = BABYLON.MeshBuilder.CreateCylinder('lRail', {
      diameter: 0.12, height, tessellation: 40,
    }, scene);
    rail.parent = root;
    rail.position.set(sx, height / 2, 0);
    rail.material = woodMat; rail.isPickable = false;
  }
  // Horizontal rungs spaced every 0.55.
  const rungs = Math.floor(height / 0.55);
  for (let i = 0; i <= rungs; i++) {
    const rung = BABYLON.MeshBuilder.CreateCylinder('lRung' + i, {
      diameter: 0.10, height: 0.78, tessellation: 12,
    }, scene);
    rung.parent = root;
    rung.rotation.z = Math.PI / 2;
    rung.position.set(0, 0.4 + i * 0.55, 0);
    rung.material = woodMat; rung.isPickable = false;
  }
  ...
  return {
    ...
    height,
    radius: 0.7,   // climb-zone radius around the ladder's center axis
  };
}
