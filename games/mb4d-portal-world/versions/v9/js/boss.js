...
function tickParticleColider(b, dt, player, cb) {
  ...
  // Fireball cadence per phase. Phase 5 is the "longest phase" torrent.
  const cadence = b.phase === 2 ? 2.5 : b.phase === 3 ? 2.0 : b.phase === 4 ? 1.6 : 0.35;
  if (b.timer <= 0) {
    let shots;
    if (b.phase === 5) shots = 5;           // wide fan of 5 per burst
  ...
  // Phase 5 lasers: a fast straight-line projectile aimed at MB. Cadence
  // ~0.8s so a few are always in flight. Lasers travel ~3x faster than
  // fireballs and hit hard.
  if (b.phase >= 5 && b.laserTimer <= 0 && cb.spawnLaser) {
    b.laserTimer = 0.8;
    const a = Math.atan2(to.x, to.z);
    const dir = new BABYLON.Vector3(Math.sin(a), 0.0, Math.cos(a)).normalize();
    cb.spawnLaser(b.center().add(new BABYLON.Vector3(0, 0.4, 0)), dir.scale(36), def.laserDmg || 20);
  }
