...
(function () {
  ...
  const Audio4 = {
    ...
    // Real announcer — speaks text via Web Speech API if available.
    // Layers a synth stab on top of it for punch. If TTS isn't available
    // (some browsers/embedded views), falls back to just the stab.
    // Args: text to speak, intensity (0..1.5 affects stab loudness).
    speak(text, intensity) {
      // Play the stab regardless — gives audible feedback even if TTS is mute
      Audio4.announceStab(intensity || 1.0);
      if (muted) return;
      // Use the browser TTS if it exists
      if (typeof window !== 'undefined' && window.speechSynthesis && window.SpeechSynthesisUtterance) {
        try {
          // Cancel any in-flight utterance so we don't queue up a backlog
          window.speechSynthesis.cancel();
          const u = new window.SpeechSynthesisUtterance(text);
          u.rate = 1.25;      // a bit faster — these are short shouts
          u.pitch = 1.15;     // slightly bright/energetic
          u.volume = 0.85;
          // If multiple voices are available, prefer en-US
          const voices = window.speechSynthesis.getVoices();
          if (voices && voices.length) {
            const pref = voices.find(v => /en-?US/i.test(v.lang)) ||
          ...
          // Silently fall back to the stab-only feedback
        }
      }
    },
  };
