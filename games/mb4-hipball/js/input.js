...
(function () {
  ...
  function tick() {
    // Player 1: WASD + Space + JKL  (and touch buttons)
    updatePlayer(p1, {
      left:  keys['a'] || keys['ArrowLeft'] || touchFlags.left,
      right: keys['d'] || keys['ArrowRight'] || touchFlags.right,
      fwd:   keys['s'] || keys['ArrowDown'],       // INTO the screen
      back:  keys['w'] || keys['ArrowUp'],         // OUT of the screen
      jump:  keys[' '] || keys['Space'] || touchFlags.jump,
      hip:   keys['j'] || touchFlags.hip,
      knee:  keys['k'] || touchFlags.knee,
      elbow: keys['l'] || touchFlags.elbow,
    });

    // Player 2 (local couch co-op fallback): numpad / arrow alternative
    // Used only when scene asks for it; multiplayer overrides this.
    updatePlayer(p2, {
      left:  keys['Numpad4'] || false,
      right: keys['Numpad6'] || false,
      fwd:   keys['Numpad5'] || false,
      back:  keys['Numpad8'] || false,
