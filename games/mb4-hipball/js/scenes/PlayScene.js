...
class PlayScene extends Phaser.Scene {
  ...
  _buildHud() {
    ...
    // Active powerups for each player — name + time-remaining bar.
    // We render multi-line; each line is "LABEL  N.Ns" with a thin bar under.
    this.hud.puP1 = this.add.text(20, 80, '', {
      fontFamily: '"Black Ops One", "Arial Black", sans-serif',
      fontSize: '13px', color: '#fde375',
      stroke: '#1a0820', strokeThickness: 3,
      lineSpacing: 2,
    }).setDepth(900);
    this.hud.puP2 = this.add.text(W - 20, 80, '', {
      fontFamily: '"Black Ops One", "Arial Black", sans-serif',
      fontSize: '13px', color: '#9adfff',
      stroke: '#1a0820', strokeThickness: 3,
      lineSpacing: 2,
      align: 'right',
  ...
  _applyPowerUp(player, kind) {
    ...
    Audio4.powerUpEarned('major');   // always a big sting now
    this._announce(def.label, 1.2);
