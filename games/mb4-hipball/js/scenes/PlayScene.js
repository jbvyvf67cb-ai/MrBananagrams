...
class PlayScene extends Phaser.Scene {
  ...
  _buildHud() {
    ...
    // Math streak — small "MATH: ●●○○" indicators below the player labels
    this.hud.mathP1 = this.add.text(20, 80, '', {
      fontFamily: '"Black Ops One", "Arial Black", sans-serif',
      fontSize: '14px', color: '#fde375',
      stroke: '#1a0820', strokeThickness: 3,
    }).setDepth(900);
    this.hud.mathP2 = this.add.text(W - 20, 80, '', {
      fontFamily: '"Black Ops One", "Arial Black", sans-serif',
      fontSize: '14px', color: '#9adfff',
      stroke: '#1a0820', strokeThickness: 3,
    }).setOrigin(1, 0).setDepth(900);

    // Active powerups — list under math streak
    this.hud.puP1 = this.add.text(20, 102, '', {
      fontFamily: '"Black Ops One", "Arial Black", sans-serif',
      fontSize: '12px', color: '#4ea886',
      stroke: '#1a0820', strokeThickness: 3,
    }).setDepth(900);
    this.hud.puP2 = this.add.text(W - 20, 102, '', {
      fontFamily: '"Black Ops One", "Arial Black", sans-serif',
      fontSize: '12px', color: '#4ea886',
      stroke: '#1a0820', strokeThickness: 3,
    }).setOrigin(1, 0).setDepth(900);
  ...
  _updateHud() {
    ...
    // Math streak dots — ● for solved, ○ for empty, capped at MAJOR (4)
    const total = MB4.MATH.STREAK_MAJOR;
    for (let pi = 0; pi < 2; pi++) {
      const s = this.players[pi].mathStreak || 0;
      const dots = '\u25CF'.repeat(s) + '\u25CB'.repeat(Math.max(0, total - s));
      const label = 'MATH ' + dots;
      this.hud['mathP' + (pi + 1)].setText(label);
    }

    // Active powerups
    for (let pi = 0; pi < 2; pi++) {
      const pups = this.players[pi].powerUps || [];
      const labels = pups.map(pu => MB4.POWERUPS[pu.kind].shortLabel).join(' ');
      this.hud['puP' + (pi + 1)].setText(labels);
    }
  }
  ...
  _showRallyStart() {
    ...
    if (!this._helpShown) {
      ...
      const helpLines = [
        ...
        'JUMP + C (elbow) to LOB at the hoop',
      ];
      const help = this.add.text(W / 2, 380, helpLines.join('\n'), {
        fontFamily: '"Black Ops One", "Arial Black", sans-serif',
        fontSize: '16px', color: '#fde375',
        stroke: '#1a0820', strokeThickness: 4,
  ...
  _resetPositions() {
    ...
    // Math tiles persist *across* rallies — they're part of the continuous
    // experience. Player math streaks also persist.
    // But clear stale tiles from the floor for visual freshness.
    if (this.mathTiles) this.mathTiles.destroyAll();
  }


  // ---- Math/powerup helpers ----

  _earnPowerUp(player, tier) {
    const pool = MB4.POWERUP_POOL[tier];
    const kind = pool[Math.floor(Math.random() * pool.length)];
    this._applyPowerUp(player, kind);
  }

  // Public API — call this to grant any powerup to any player.
  _applyPowerUp(player, kind) {
    ...
    Audio4.powerUpEarned(def.tier);
    this._announce(def.label, def.tier === 'major' ? 1.4 : 1.0);
    // visual pop on the player
  ...
  _aggregateMods(player) {
    ...
    return { moveMaxMult, moveAccelMult, bodyBounceMult, strikeBoost };
  }
  ...
  _updateBall(dt, time) {
    ...
    // ---- Body collisions ----
    // The ball doesn't pass through players. If it overlaps a player's
    // body cylinder, we reflect it off as a "body block" — different from a
    // strike: no power play, streak doesn't count, but the ball doesn't
    // sail through unblocked. Last-touched-by is cleared so neither player
    // can claim a hoop shot off a passive body bounce.
    ...
    if (b.lastTouchedBy !== null && b.depth > 0.70 && !this._hoopJustScored) {
      ...
      if (Math.abs(dy) < MB4.HOOP_RADIUS_V) {
        ...
        if (prevSide * currSide <= 0 && Math.abs(b.vx) > 10) {
          return this._hoopScore(b.lastTouchedBy);
        }
        if (Math.abs(currSide) < MB4.HOOP_PASS_THICKNESS) {
          return this._hoopScore(b.lastTouchedBy);
        }
      }
    }
  ...
  // Hoop = +3 rayas (was instant-win). Don't end the rally — just credit
  // the rayas and let play continue, unless the score threshold is hit.
  _hoopScore(scorerIdx) {
    // Guard against multi-frame re-fire — the ball might still be near the hoop
    // for several frames after the actual cross.
    this._hoopJustScored = true;
    this.time.delayedCall(1500, () => { this._hoopJustScored = false; });

    this.match.score[scorerIdx] += MB4.HOOP_RAYAS;
    Audio4.hoopWin();
    this._announce(`HOOP! +${MB4.HOOP_RAYAS}`, 1.5);

    // Big flame burst at the hoop
    for (let i = 0; i < 40; i++) {
    ...
    // Reset the ball off the hoop so it doesn't keep re-triggering
