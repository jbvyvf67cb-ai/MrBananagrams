...
class PlayScene extends Phaser.Scene {
  ...
  _checkScoring(time) {
    const b = this.ball;
    // Hoop pass-through. The ball must be deep enough into the court that
    // it's near/at the back wall where the ring lives, and at the right
    // screen-Y to be inside the ring vertically.
    if (MB4.HOOP_INSTAWIN && b.lastTouchedBy !== null && b.depth > 0.70) {
      const floorY = MB4.FLOOR_FRONT_Y + (MB4.FLOOR_BACK_Y - MB4.FLOOR_FRONT_Y) * b.depth;
      const screenY = floorY - b.height;
      const dy = screenY - MB4.HOOP_Y;
      if (Math.abs(dy) < MB4.HOOP_RADIUS_V) {
        // Crossed the hoop X plane this frame? (most common case)
        ...
        // Ball lingering inside the ring (slow-passing case)
