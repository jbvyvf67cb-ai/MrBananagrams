...
class PlayScene extends Phaser.Scene {
  ...
  _drawBackdrop() {
    ...
    [MB4.HOOP_LEFT_X, MB4.HOOP_RIGHT_X].forEach((hx) => {
      const skyDisc = this.add.graphics();
      skyDisc.fillStyle(C.skyMid, 1);
      skyDisc.fillCircle(hx, MB4.HOOP_Y, 17);
      const hoop = this.add.image(hx, MB4.HOOP_Y, 'hoop').setOrigin(0.5).setScale(1.0);
      hoop.setDepth(5);
    });
  ...
  _checkScoring(time) {
    const b = this.ball;
    if (MB4.HOOP_INSTAWIN && b.lastTouchedBy !== null && b.height > 100) {
      const targetX = b.lastTouchedBy === 0 ? MB4.HOOP_RIGHT_X : MB4.HOOP_LEFT_X;
      const floorY = MB4.FLOOR_FRONT_Y + (MB4.FLOOR_BACK_Y - MB4.FLOOR_FRONT_Y) * b.depth;
      const screenY = floorY - b.height;
      const dx = b.x - targetX;
      const dy = screenY - MB4.HOOP_Y;
      if (Math.abs(dx) < MB4.HOOP_RADIUS && Math.abs(dy) < MB4.HOOP_RADIUS && b.depth > 0.7) {
