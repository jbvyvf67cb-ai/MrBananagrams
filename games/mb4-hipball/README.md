...
The game looks for short MP3 voice clips in `audio/voice/`. When a powerup activates or a key event happens, it tries to play the matching clip. If the file is missing, it falls back to event-specific synth noises — a rising horn for powerups, a descending blast for rally start, a quick whoop for scores. **No speech-imitation, no robot voices.** Sounds intentional and arcade-y out of the box.
...
### How to add real voice

The simplest path: open your phone's voice memo app, shout each name with maximum arcade-announcer gusto ("JAGUAR STEP!", "TWIN SUN!"), export as MP3, drop in the corresponding file. The game picks them up automatically — no code changes.

For generic events, [**VoiceBosch's "Mr. Happy" pack**](https://opengameart.org/content/mr-happy-announcer-audio-pack) on OpenGameArt (CC-BY-SA 4.0) has high-energy "Fight!" / "Victory!" / "You Win!" phrases you can rename to match.
